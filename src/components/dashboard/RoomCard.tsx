...
export function RoomCard({ room, index = 0 }: { room: Room; index?: number }) {
  return (
    <GlassPanel
      className="group overflow-hidden opacity-0 animate-fade-in-up transition-all duration-300 hover:-translate-y-1 hover:bg-black/[0.07]"
    ...
    >
      <div className={cn("relative aspect-video", room.posterClassName)}>
        ...
        >
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-black/20 backdrop-blur-md transition-transform hover:scale-110">
      ...
      <div className="p-4">
        ...
        <Link
          href={`/room/${room.id}`}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-black/10 py-2 text-sm font-medium text-foreground transition-colors hover:bg-black/10"
        >
          {room.isLive ? (
            <>
